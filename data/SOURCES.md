...
The rounded coordinates are retained in the processed source for provenance, but they are not exported as demand points. During demand generation, each prepared workplace is assigned to its nearest census-derived 0.02-degree analysis zone and its capacity is summed there. This removes false sub-block precision and gives residences and jobs the same spatial resolution without random coordinate jitter.

Demand first balances formal workplace capacity against scaled residence-based employed residents with a distance-constrained iterative proportional fitting model. A gravity-bounded maximum-flow pass identifies a feasible support, and a minimum-surprisal transportation solve projects the dense matrix onto an exact sparse set of flows. The difference between total employed residents and formal workplace capacity is represented as local residual employment at residential origins because the source does not locate those jobs.

Exported job points use census-derived analysis-zone coordinates and may contain more than 200 jobs. Only individual population records are split to at most 200 people for game compatibility. Residence, workplace, and local-employment roles in the same zone share one demand point.
